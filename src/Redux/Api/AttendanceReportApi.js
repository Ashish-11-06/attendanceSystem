...
export const fetchAttendanceReportById = async (event_id) => {
  ...
  try {
    ...
    return { event_id, unit_summary: response.data.unit_summary || [] };
  } catch (error) {
...
// Fetch reports from ID 1 to 100 (or a larger number if needed)
  ...
  for (let event_id = 1; event_id <= maxEventId; event_id++) {
    try {
      ...
      if (report && report.unit_summary.length > 0) {  
        ...
        console.log(`No data for event ID ${event_id}, skipping.`);
      }
    } catch (error) {
